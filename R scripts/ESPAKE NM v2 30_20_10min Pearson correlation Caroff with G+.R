...
features.Car <- features[1:11,]
features.NM <- features[12:22,]   # change according to how many spectra you imported!
#--------------------------------------------------------------------------------------------------------


# rename the rowname of features, to simply the bacterial species name

rownames(features.Car) <- c("Caroff A. baumannii mcr*",
                         "A. baumannii WT",
                         "E. cloacae",
                        "E.coli mcr*",
                        "E.coli WT",
...
rownames(features.NM) <- c("NM A. baumannii mcr*",
                           "A. baumannii WT",
                           "E. cloacae",
                           "E.coli mcr*",
                           "E.coli WT",
...
library(corrplot)
library(Hmisc)
col<- colorRampPalette(c( "blue","white","tomato2"))(256)
...
p.mat<- p.mat.all[1:11, 12:22]  # change according to how many spectra you imported!
...
corrplot(sim, method="circle", main=NULL,
        bg="ghostwhite", tl.col="black", tl.srt = 50, col=col, tl.cex = 1.1, tl.pos = "lt",
         number.digits = 1,cl.cex = 1.0, mar=c(0,0,1,0), cl.lim = c(-0.25,1), 
